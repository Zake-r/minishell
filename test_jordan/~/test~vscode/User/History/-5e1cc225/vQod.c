...
void    builtin_echo(t_ast *ast)
{
    ...
    if (ast->args[1] && ft_strcmp(ast->args[1], "-n") == 0)
    {
        newline = 0;
        i = 2;
    }
...
void    exec_builtin(t_ast *ast, char **env)
{
    if (ft_strcmp(ast->args[0], "cd") == 0)
        ...
        builtin_pwd();
    else if (ft_strcmp(ast->args[0], "echo") == 0)
        builtin_echo(ast);
    else if (ft_strcmp(ast->args[0], "export") == 0)
        builtin_export(ast, env);
    else if (ft_strcmp(ast->args[0], "unset") == 0)
        builtin_unset(ast);
    else if (ft_strcmp(ast->args[0], "env") == 0)
        builtin_env(env);
